"""
SME Compliance Monitor — Report Generator
------------------------------------------
Produces audit-grade multi-page PDF compliance reports.
Supports client branding — client name appears in header,
title block, footer, and certification section.
...
class CompliancePDF(FPDF):
    ...
    def header(self):
        ...
        self.set_y(5)
        client_str = f" -- {self.client_name.upper()}" if hasattr(self, "client_name") and self.client_name else ""
        self.cell(0, 8, _s(f"SME COMPLIANCE MONITOR - AUDIT REPORT{client_str}"), align="C")
    ...
    def footer(self):
        ...
        client_str = f" | Client: {self.client_name}" if hasattr(self, "client_name") and self.client_name else ""
        self.cell(0, 5, _s(f"CONFIDENTIAL{client_str} -- Generated {datetime.now().strftime('%d %b %Y %H:%M')} | Page {self.page_no()}"), align="C")
    ...
    def section_title(self, title: str):
        ...
        self.cell(0, 8, f"  {_s(title)}", fill=True, ln=True)
        self.set_text_color(*BRAND_DARK)
        self.ln(2)
...
def generate_report(violations: list, risk_summary: dict, posture_scores: dict,
                    output_path: str, client_name: str = "") -> str:
    """
    Generate a full compliance audit PDF report.
    client_name — if provided, appears throughout the report.
    Returns the output file path.
    """
    pdf = CompliancePDF(orientation="P", unit="mm", format="A4")
    pdf.set_auto_page_break(auto=True, margin=18)
    pdf.set_margins(15, 22, 15)
    pdf.client_name = client_name.strip() if client_name else ""
    pdf.add_page()

    # ── Client Banner ─────────────────────────────────────────────────────────
    if pdf.client_name:
        pdf.set_fill_color(*CLIENT_BLUE)
        pdf.set_font("Helvetica", "B", 12)
        pdf.set_text_color(*BRAND_BLUE)
        pdf.cell(0, 11, _s(f"CLIENT: {pdf.client_name.upper()}"), fill=True, ln=True, align="C")
        pdf.ln(3)
    ...
    if pdf.client_name:
        pdf.cell(0, 6, _s(f"Prepared for: {pdf.client_name}"), ln=True, align="C")
    ...
    values  = [str(risk_summary.get(k, 0)) for k in ["Critical","High","Medium","Low","Total"]]
    ...
    for fw_key, fw_name in framework_labels.items():
        ...
        bar_col   = LOW_GREEN if score >= 70 else MEDIUM_YELLOW if score >= 40 else CRITICAL_RED
    ...
    for v in violations[:40]:
        ...
        pdf.cell(widths[5], 6, gdpr, fill=True, border=1)
        pdf.ln()
        fill = not fill
    ...
    recommendations = [
        ("CM-001 -- Unauthorised Access",
         "Deploy account lockout after 5 failed attempts. Review Kerberos pre-authentication. Enable SIEM alerting on brute force patterns (ISO 27001 A.8.3)."),
        ("CM-002 -- Unencrypted Transmission",
         "Enforce TLS 1.2/1.3 for all data-in-transit. Block HTTP/FTP on the perimeter firewall. Review DLP policies (GDPR Art. 32)."),
        ("CM-003 -- Excessive Privileges",
         "Implement PAM. Enforce least privilege and Just-In-Time access. All admin assignments require a change ticket (ISO A.8.2)."),
        ("CM-004 -- Missing Patches",
         "Establish automated patching with 14-day SLA for Critical CVEs per Cyber Essentials. Run weekly vulnerability scans."),
        ("CM-005 -- Data Retention Breach",
         "Deploy automated retention enforcement aligned to documented schedules. Conduct quarterly data minimisation audit (GDPR Art. 5(1)(e))."),
        ("CM-006 -- No MFA",
         "Mandate MFA for all cloud and remote-access accounts. Use conditional access policies in Azure Entra ID."),
        ("CM-007 -- Audit Log Issues",
         "Centralise logging to immutable SIEM. Alert on log gaps > 15 minutes. Protect integrity with WORM storage (ISO A.8.15)."),
        ("CM-008 -- Firewall Misconfiguration",
         "Conduct immediate firewall rule review. Implement change management for all rule modifications. Block RDP/SSH from internet unless via VPN (Cyber Essentials)."),
    ]
    ...
    pdf.set_font("Helvetica", "", 9)
    if pdf.client_name:
        pdf.cell(0, 6, _s(f"  Client:              {pdf.client_name}"), ln=True)
    pdf.cell(0, 6, f"  Report generated by: SME Compliance Monitor v2.0.0", ln=True)
    pdf.cell(0, 6, f"  Prepared by:         Opoku Mensah | Cybersecurity Consultant", ln=True)
    pdf.cell(0, 6, f"  Frameworks assessed: UK GDPR/DPA 2018 | ISO/IEC 27001:2022 | NIST CSF 2.0 | Cyber Essentials", ln=True)
    pdf.cell(0, 6, f"  Total violations:    {len(violations)}", ln=True)
